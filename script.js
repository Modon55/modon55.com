...
if (letters.length) {
  ...
  if (window.location.protocol === 'file:') {     // Local editor
...
document.querySelectorAll('.btn').forEach(btn => {
  ...
  function animate() {
    ...
    btn.style.setProperty('--mx', `${currentX}px`);
    btn.style.setProperty('--my', `${currentY}px`);

    requestAnimationFrame(animate);
  }

  currentX = btn.offsetWidth / 2;
  currentY = btn.offsetHeight / 2;
  targetX = currentX;
  targetY = currentY;
